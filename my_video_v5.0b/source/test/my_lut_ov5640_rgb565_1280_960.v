...
module lut_ov5640_rgb565_1280_960(
...
begin
	case(lut_index)			  
		...
		10'd 65: lut_data <= {8'h78 , 24'h3a1f14};// fast zone low
        // Lens correction for
		...
		10'd186: lut_data <= {8'h78 , 24'h538b98}; // sign[8:1] 
        // UV adjust
		...
		10'd212: lut_data <= {8'h78 , 24'h382106};// Sensor mirror on, ISP mirror on, H binning on
		...
		10'd227: lut_data <= {8'h78 , 24'h380c07};// HTS            //Total horizontal size 800 包含消隐区的总像素
		...
		10'd251: lut_data <= {8'h78 , 24'h303521};// PLL     input clock =24Mhz, PCLK =96Mhz
		10'd252: lut_data <= {8'h78 , 24'h303669};// PLL  改到78=96MHz,69=84MHz,OK
		10'd253: lut_data <= {8'h78 , 24'h3c0707}; // lightmeter 1 threshold[7:0]
		10'd254: lut_data <= {8'h78 , 24'h382041}; // flip  //改为flip off
		10'd255: lut_data <= {8'h78 , 24'h382106}; // mirror//改为mirror on
		10'd256: lut_data <= {8'h78 , 24'h381431}; // timing X inc
		10'd257: lut_data <= {8'h78 , 24'h381531}; // timing Y inc
		10'd258: lut_data <= {8'h78 , 24'h380000}; // HS x起始位置
		10'd259: lut_data <= {8'h78 , 24'h380100}; // HS
		10'd260: lut_data <= {8'h78 , 24'h380200}; // VS y起始位置
		10'd261: lut_data <= {8'h78 , 24'h380304}; // VS
		10'd262: lut_data <= {8'h78 , 24'h38040a}; // HW (HE) 传感器宽（x结束位置）
		10'd263: lut_data <= {8'h78 , 24'h38053f}; // HW (HE)
		10'd264: lut_data <= {8'h78 , 24'h380607}; // VH (VE) 传感器高（y结束位置）
		10'd265: lut_data <= {8'h78 , 24'h38079f}; // VH (VE)
		10'd266: lut_data <= {8'h78 , 24'h380805}; // DVPHO     1280 输出宽
		10'd267: lut_data <= {8'h78 , 24'h380900}; // DVPHO
		10'd268: lut_data <= {8'h78 , 24'h380a03}; // DVPVO     960   输出高
		10'd269: lut_data <= {8'h78 , 24'h380bc0}; // DVPVO
		10'd270: lut_data <= {8'h78 , 24'h380c07}; // HTS 用来调整帧率（改动）
		...
		10'd298: lut_data <= {8'h78 , 24'h350300}; // AEC/AGC on 自动曝光控制、自动增益控制
