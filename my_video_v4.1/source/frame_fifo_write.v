...
//根据写入请求write_req，管理ddr3层写入所需的各类信号(wr_burst_req,wr_burst_len,wr_burst_addr)
`timescale 1ns/1ps
...
(
	...
	input                            write_req,                  // data write module write request,keep '1' until read_req_ack = '1' 帧写入请求
...
begin
	...
	begin
		...
		case(state)
			...
			//'S_ACK' 状态完成写请求响应，复位FIFO。锁定地址、数据长度
			S_ACK:
			begin
				//after write request revocation(write_req_d2 == '0'),goto 'S_CHECK_FIFO',write_req_ack goto '0'
				//写请求结束后，goto 'S_CHECK_FIFO',write_req_ack goto '0'
				if(write_req_d2 == 1'b0)//握手完成
				begin
					state <= S_CHECK_FIFO;
					fifo_aclr <= 1'b0;//FIFO的异步清除信号结束
					write_req_ack <= 1'b0;
				end
				else
				begin
			...
			begin
				...
				begin
					...
					wr_burst_req <= 1'b1;//fifo储存够一个burst长度，请求写入ddr3
				end
			end
			...
			begin
				...
				begin
					...
					write_cnt <= write_cnt + BURST_SIZE[ADDR_BITS - 1:0];//写入了BURST_SIZE*4个像素
					//the next burst write address is generated
					wr_burst_addr <= wr_burst_addr + BURST_SIZE[ADDR_BITS - 1:0];//!!!!!!!注意，BURST_SIZE个地址，每个地址上有4个像素
				end     
			end
			S_WRITE_BURST_END:
			begin
				...
				else if(write_cnt < write_len_latch)//已写入的长度比较 帧长度（帧长度=像素/4）
				begin
					state <= S_CHECK_FIFO;
				end
				else
