...
(
    ...
    output [1:0]                        read_cnt,//
    input                            ddr3_rd_ready    // from external memory controller
);
	...
	begin
		...
		case(state)
			...
			begin
				...
				else if(wrusedw < (FIFO_DEPTH - BURST_SIZE - 2))//同时要ddr3_rd_ready准备好
				begin
					state <= S_READ_BURST;
					rd_burst_len <= BURST_SIZE[BUSRT_BITS - 1:0];
					rd_burst_req <= 1'b1;//发出读请求
				end
			end
			...
			begin
				if(rd_burst_data_valid)
					rd_burst_req <= 1'b0;//数据有效后读请求拉低
				//burst finish  
				if(rd_burst_finish == 1'b1)
				begin
					state <= S_READ_BURST_END;
                    read_cnt <= read_cnt + 2'd1;//改
                    if(read_cnt == 2'd3)
                    begin
